...
class DispenserControlConsumer(AsyncWebsocketConsumer):
    ...
    def update_connectivity(self, imei: str, status: str):
        """
        Update DispenserUnits and related gun mappings connectivity_status.
        Applies to both Customer and Vehicle mappings wherever present.
        """
        try:
            dispenser = DispenserUnits.objects.get(imei_number=imei)
        except DispenserUnits.DoesNotExist:
        ...
        # Update dispenser itself
        dispenser.connectivity_status = is_online
        dispenser.save(update_fields=["connectivity_status"])

        # Update customer mappings (if any)
        Dispenser_Gun_Mapping_To_Customer.objects.filter(
            dispenser_unit_id=dispenser.id
        ).update(connectivity_status=is_online)

        # Update vehicle mappings (if any)
    ...
    def update_machine_status(self, imei: str, status: str):
        ...
        try:
            ...
            affected = Dispenser_Gun_Mapping_To_Customer.objects.filter(
                ...
                # Update vehicle mappings (if any)
            vehicle_count = Dispenser_Gun_Mapping_To_Vehicles.objects.filter(
                dispenser_unit_id=dispenser.id
            ).update(machine_status=status)

            print(f"[MACHINE STATUS] Updated {affected}/{vehicle_count} mapping(s) for IMEI {imei} to status {status}")
    ...
    def save_totalizer_and_gps_starting_with_validation(self, transaction_id, imei, totalizer_vol, totalizer_price, gps_data):
        txn = None
        txn_source = None  # "request" or "order"
        ...
        # ✅ Find last_txn in the SAME table where txn came from
        if txn_source == "request":
            last_txn = (
        ...
        else:  # txn_source == "order"
            last_txn = (
                OrderFuelDispensingDetails.objects
                .filter(dispenser_gun_mapping_id=dispenser_gun_mapping_id)
                .exclude(id=txn.id)
                .order_by("-request_created_at")
                .first()
    ...
    def update_transaction_log(self, data):
        ...
        if request is None:
            try:
                request = OrderFuelDispensingDetails.objects.get(transaction_id=transaction_id)
